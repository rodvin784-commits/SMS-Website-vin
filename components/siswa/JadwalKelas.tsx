...
import { BookOpen, CalendarX2, Clock, MapPin, User } from 'lucide-react'
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 flex flex-col items-center justify-center space-y-3">
        ...
        <p className="text-sm font-medium text-gray-500">Memuat jadwal kelas...</p>
      </div>
    )
  }
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
        ...
        <p className="text-sm text-gray-500 max-w-md mx-auto">
          Jadwal untuk kelas Anda belum diatur oleh admin.
        </p>
      </div>
    )
  }
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span className={`text-xs font-bold ${entries.length > 0 ? 'text-blue-600' : 'text-gray-400'}`}>
                {entries.length} sesi
              </span>
            </div>
            ...
            <div className="p-3 space-y-2.5 min-h-[120px]">
              {entries.length === 0 ? (
                <p className="text-xs text-gray-400 italic text-center pt-6">Tidak ada pelajaran</p>
              ) : (
                entries.map((entry) => (
                  <div key={entry.id} className="rounded-xl border border-blue-100 bg-blue-50/60 p-3 space-y-1.5">
                    ...
                    <div className="flex items-start gap-1.5">
                      ...
                      <div>
                        ...
                        {entry.mapel_kode && (
                          <p className="text-xs text-gray-500 font-semibold uppercase">{entry.mapel_kode}</p>
                        )}
                      </div>
                    </div>
                    {entry.guru_nama && (
